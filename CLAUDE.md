...
| `bank_statements` | **super_admin only** can insert. **Upload UI moved to `/reconciliation` (2026-05-30)** and is now shown to **super_admin only** — FA/SM no longer see the section at all (RLS still lets them SELECT, they just have no UI for it). super_admin delete; FA + SM + super_admin select. | `uploaded_by` FK→profiles, `file_path` (Storage), `period_start` / `period_end` filled by extractor. One row per uploaded statement PDF. |
