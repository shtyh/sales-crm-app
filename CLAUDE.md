...
| `cars` | per-column gated by trigger; delete super only (UI exposed at `/cars/:id` ★ Delete; two-step chassis-typed confirm; bookings.car_id is `on delete set null` so deletion never blocks) | `chassis_no unique`, `floor_stock_*`, `status enum(in_stock/reserved/delivered/returned)`. **Floor-stock UI (2026-05-30):** the `/cars/:id` section is labelled **🏦 Inventory financing** (matches FinancePage) with a tight bank dropdown — **Public Bank or Cash** only (not the 13-bank customer-loan list). `floor_stock_bank` now stores the literal `'Cash'` for cash-funded cars (was NULL); NewCarPage's cash funding writes `'Cash'` + `paid_off`. Legacy/other bank values still render via a fallback `<option>`. |
