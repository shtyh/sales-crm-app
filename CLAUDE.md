...
|---|---|
| `customer_*`, `vehicle_*`, `otr_price`, `booking_fee`, `down_payment`, `booking_date`, `notes` | booking owner (SA) + any privileged role (general_admin / sales_manager / finance_admin). `down_payment` (2026-05-30) = the agreed customer down payment entered in the Pricing form; NOT guard-gated; distinct from `total_received_down_payment` (auto-summed from receipts). |
...
| `special_support` | sales_manager only — RM bonus that adds to commission. **UI removed from the booking Pricing form 2026-05-30** (column + commission math kept for legacy rows; the commission breakdown only shows the "+ Special support" card when it's non-zero). |
...
the AI doc cards are gated to SA/SM/super via `showDocCards`. Layout inside the
box: **All-In-One spans the full width** (its own row), then **Down payment · LOU
· 🏦 Bank transaction share a `md:grid-cols-3` row** (the bank slot sits in the
3rd column). When `showDocCards` is false (finance/general) only the bank slot
renders. Then at the bottom: 💳 Bank statement + ❌ Cancellation form. The old 📃 LOU `AttachmentSection`
